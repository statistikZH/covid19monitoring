...
url_Mobility_SBBHauptbahnhof <-"https://raw.githubusercontent.com/statistikZH/covid19monitoring_mobility_SBBHauptbahnhof/master/Mobility_SBBHauptbahnhof.csv"
Mobility_SBBHauptbahnhof <- read.csv(url(url_Mobility_SBBHauptbahnhof), header=T, sep=",", stringsAsFactors=FALSE, encoding="UTF-8")

###########################################################################
# rbind

covid19monitoring <- rbind(Other_Weather, Mobility_VBZHardbruecke, Mobility_Intervista, Economy_SMI, Economy_FlightTrafficZRH, Economy_SIX, Education_Mindmaps, Education_LMVZ, Health_Covid19Cases, Economy_KOF, Mobility_SBBHauptbahnhof)
...
```
## Mobility_SBB_Hardbrücke

```{r, echo = FALSE}

CheckIfDfFollowsStandard1(Mobility_SBBHauptbahnhof)
CheckIfDfFollowsStandard2(Mobility_SBBHauptbahnhof)

Mobility_SBBHauptbahnhof$date <- as.POSIXct(Mobility_SBBHauptbahnhof$date)
...
# Mobility 1: SBB
ggplot(data = Mobility_SBBHauptbahnhof2, aes(x = date, y = value )) + 
   geom_line(color = "#00AFBB", size = 1.0, alpha = 1) +
   scale_x_date(date_labels = "%a %d. %b") +
  labs(title = "ÖV-Besucherfrequenzen, Zählstelle Hardbrücke, Unit: Anz. in 1'000",
       y = "Besucherfreq. (Anz. in 1'000)")
...
Education_LMVZ <- subset(data, description == "https://bildungsmonitoringzh.github.io/covid19_edu_lmvzmath/")
